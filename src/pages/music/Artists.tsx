...
export default function ArtistsId() {
  ...
  const Artits_MV = () => {
    return artist?.sections?.find((item) => item.sectionType === 'video')
  }

  const Artits_Playlist = () => {
    return artist?.sections?.find((item) => item.sectionType === 'playlist')
  }
    ...
    <div className='text-white w-full min-h-screen pb-24'>
        ...
        <div className='pt-20 pb-16 px-8 flex flex-col md:flex-row items-center lg:items-end gap-8 relative z-20'>
          ...
          <div className='shrink-0 group'>
            <div className='relative'>
              ...
              {/* Verified Badge */}
              <div className='absolute bottom-0 right-6 rounded-full p-3 '>
                <Icon name='verified' size={50} className='text-white text-2xl' />
              </div>
          ...
          <div className='flex flex-col justify-end space-y-6 flex-1'>
            ...
            <div className='flex items-center gap-6 pt-6 flex-wrap'>
              ...
              <button className='text-gray-400 hover:text-white hover:scale-110 hover:bg-white/10 p-4 rounded-full transition-all duration-300'>
                <Icon name='heart' size={20} className='text-2xl' />
              </button>
      ...
      <div className='md:px-8 mt-8'>
        <div className='bg-black/30 backdrop-blur-sm rounded-2xl p-6 border border-white/10'>
          ...
          {(Artits_Playlist()?.items?.length ?? 0) ? (
            <Swiper
              spaceBetween={24}
              slidesPerView={2}
              loop={(Artits_Playlist()?.items?.length ?? 0) > 2}
            ...
            >
              {Artits_Playlist()?.items.map((album) => (
                  ...
                  <div className='group bg-white/5 rounded-2xl p-4  hover:bg-white/10 transition-all duration-300 cursor-pointer h-70'>
                    <div className='relative mb-4'>
                      ...
                      <div className='absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300'>
                        <button
                          className='bg-green-500/90 backdrop-blur-sm text-black p-3 rounded-full hover:bg-green-400 hover:scale-110 transition-all duration-300 shadow-xl '
                          onClick={() => {
      ...
      <div className='md:px-8 mt-8'>
        <div className='bg-black/30 backdrop-blur-sm rounded-2xl p-6 border border-white/10'>
          <div className='flex items-center justify-between mb-6'>
            ...
            {(Artits_MV()?.items?.length ?? 0) > 6 && (
              <button className='text-green-400 hover:text-green-300 font-semibold flex items-center gap-2 group'>
                <Link to={`/arist/mv?name=${artist.name}`}>
                  <span>Xem tất cả</span>
                  <Icon name='arrow-right' className='text-lg group-hover:translate-x-1 transition-transform' />
                </Link>
              </button>
          ...
          <div className='overflow-x-auto scrollbar'>
            <div className='flex gap-6 pb-4' style={{ minWidth: 'max-content' }}>
              {Artits_MV()?.items.map((video) => (
                <Link
                  key={video.encodeId}
                  to={`/video?id=${video.encodeId}`}
                  className='group relative hover:scale-105 transition-transform duration-300 cursor-pointer flex-shrink-0'
                  style={{ width: '280px' }}
                >
                  {/* Video Thumbnail */}
                  <div className='relative mb-4 rounded-2xl overflow-hidden'>
                    ...
                    <div className='absolute bottom-2 right-2 bg-black/80 text-white text-xs px-2 py-1 rounded'>
                      {video.duration ? formatDuration(video.duration) : '3:45'}
                    </div>
                  ...
                  <div className='flex gap-3'>
                    <img
                      src={video.artist?.thumbnail || artist.thumbnailM}
                      alt={video.artist?.name || artist.name}
                      className='w-10 h-10 rounded-full flex-shrink-0'
                    />
                    <div className='flex-1 min-w-0'>
                      <h3 className='font-bold text-white line-clamp-2 group-hover:text-green-300 transition-colors leading-tight text-sm'>
                        {video.title}
                      </h3>
                      <p className='text-xs text-gray-400 mt-1 line-clamp-1'>
                        {Array.isArray(video.artists)
                          ? video.artists.map((artist: any) => artist.name).join(', ')
                          : video.artistsNames || 'Unknown Artist'}
                      </p>
                      <p className='text-xs text-gray-500 mt-1'>{Math.floor(Math.random() * 10) + 1}M lượt xem</p>
                    </div>
                  </div>
                </Link>
              ))}
