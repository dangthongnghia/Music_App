      ...
      <div className='p-4 lg:p-6'>
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6'>
          <div className='flex-1 min-w-0'>
            <h1 className='text-lg sm:text-xl lg:text-2xl font-bold line-clamp-2 mb-2'>{videoInfo?.title}</h1>
            <div className='flex flex-wrap items-center gap-1 text-sm text-gray-300'>
              {videoInfo?.artists?.map((artist: any, i: number) => (
                <span key={artist.id} className='hover:underline'>
                  ...
                  <Link to={`/artist?name=${artist.alias}`} className='hover:text-green-400 transition-colors'>
                    {artist.name}
                  </Link>
                </span>
              ))}
          ...
          <div className='flex items-center gap-3 flex-shrink-0'>
            <button className='p-2 text-gray-400 hover:text-red-400 transition-colors w-10 h-10'>
              <Icon name='heart' size={20} className='w-6 h-6' />
            </button>
            ...
            <div className='mt-4 lg:hidden'>
              <div className='bg-gray-800/50 rounded-lg p-4'>
                ...
                <div className='text-sm text-gray-300 space-y-2'>
                  <p>
                    <span className='text-gray-400'>Thể loại:</span> {videoInfo?.genreNames?.join(', ')}
                  </p>
                  <p>
                    <span className='text-gray-400'>Phát hành:</span>{' '}
                    {videoInfo?.releaseDate ? new Date(videoInfo.releaseDate * 1000).toLocaleDateString('vi-VN') : ''}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className='w-full lg:w-80 xl:w-96 '>
            <div className='bg-gray-800/30 rounded-lg p-4 '>
              ...
              <div className='lg:space-y-3'>
                <div className='flex lg:flex-col gap-3 overflow-x-auto lg:overflow-x-visible lg:overflow-y-auto lg:max-h-[500px] pb-2 lg:pb-0 scrollbar'>
