...
export function NewReleaseSection() {
  ...
  const [activeSection, setActiveSection] = useState<'all' | 'others' | 'vPop'>('all') 
  useEffect(() => {
   ...
   const getPlaylistSection = [
    {
      name: 'all',
      label: 'Tất cả',
      items: getNewReleaseSection()?.items?.all || []
    },
    {
      name: 'vPop',
      label: 'Việt Nam',
      items: getNewReleaseSection()?.items?.vPop || []
    },
    {
      name: 'others',
      label: 'Quốc tế', 
      items: getNewReleaseSection()?.items?.others || []
    }
  ]


  const getCurrentSectionData = () => {
    const currentSection = getPlaylistSection.find(section => section.name === activeSection)
  ...
  return (
    <div className='max-w-7xl mx-auto'>
      ...
      <div className='flex items-center gap-6 mb-8'>
        {getPlaylistSection.map((section) => (
          <div key={section.name}>
            <Button 
              className={`text-sm font-semibold mb-4 px-2 py-2 rounded-full transition-all duration-300 ${
                activeSection === section.name 
                  ? 'text-white bg-purple-500/20 border border-purple-400' 
                  : 'text-gray-400 hover:text-white hover:bg-white/10'
              }`}
              onClick={() => setActiveSection(section.name as 'all' | 'others' | 'vPop')}
            >
              {section.label}
            </Button>
          </div>
      ...
      {/* Hiển thị dữ liệu của section hiện tại */}
        ...
        {getCurrentSectionData()
          ...
          .map((song: any, index: number) => {
            return (
              ...
              >
                {/* ...existing code cho việc hiển thị song... */}
                <div className='relative mb-4'>
                  ...
                  >
                    <button
                      ...
                      onClick={() => handlePlaySongById(song.encodeId, getCurrentSectionData())}
                    >
